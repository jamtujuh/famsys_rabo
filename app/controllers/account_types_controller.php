...
class AccountTypesController extends AppController {
	...
	function index() {
		...
		$this->paginate = array('order'=>'AccountType.id');
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->AccountType->save($this->data)) {
				$this->Session->setFlash(__('The account type has been saved', true), 'default', array('class'=>'ok'));
				$this->redirect(array('action' => 'index'));
			} else {
	...
	function delete($id = null) {
		...
		if ($this->AccountType->delete($id)) {
			$this->Session->setFlash(__('Account type deleted', true), 'default', array('class'=>'ok'));
			$this->redirect(array('action'=>'index'));
		}
