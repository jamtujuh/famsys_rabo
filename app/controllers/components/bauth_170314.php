		...
		*/
		...
		$conditions[$this->user_model . ".aktif" ] = 1;
		 ...
		 if($totalUsers == 1) {
			...
			$this->Session->setFlash(__('Login Successfull.', true) . '<br>' . $change_password_message, 'default', array('class'=>'ok'));
			...
			if($attemp_left>0)
			{
				$msg =  __('Login Failed: Invalid username or password or User inactive. You have ', true);
				$msg .= $attemp_left;
				$msg .= __(' login attemps left', true);
			}
			else
			{
				$msg =  __('Login Failed: Your account is disabled. Please contact Administrator', true);
				$this->controller->{$model}->query('update users set aktif=0 where username="'.$user.'"');
			}
