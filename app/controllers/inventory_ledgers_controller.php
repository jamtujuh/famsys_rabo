...
class InventoryLedgersController extends AppController {
	...
	{
		/*print_r($id);echo "<br />";
		echo $this->configs['journal_cut_off'];
		echo "<br />";
		echo date('H:i:s');
		if($doc == 'inlog' && $this->configs['journal_cut_off'] > date('H:i:s')) echo "masuk";
		echo "<br />";echo "<br />";*/
		/****************************
		//source from inlog
		****************************/
		...
		{		
			...
			if( !empty( $inlogDetails ) ) {
				...
				if($inlog['Inlog']['inlog_status_id']==status_inlog_approved_id){
					...
					$journal = $this->InventoryLedger->insert_to_journal_csv($doc, $id, $this->RecordReferer->getIdByModelNameAndRefName('JournalGroup', 'journal_group_inlog_id'));
			...
			if( !empty( $supplierReturDetails ) ) {
				...
				foreach ($supplierReturDetails as $supplierReturDetail)
				{
					...
					if($supplierRetur['SupplierRetur']['supplier_retur_status_id']==status_supplier_retur_approved_id){
						$journal = $this->InventoryLedger->insert_to_journal_csv($doc, $id, $this->RecordReferer->getIdByModelNameAndRefName('JournalGroup', 'journal_group_supplier_retur_id'));
						$this->generateCsv($journal);
						if($supplierRetur['SupplierRetur']['retur_id'] != null){
			...
			if( !empty( $supplierReplaceDetails ) ) {
				...
				$supplier_retur_id = $supplierReplace['SupplierReplace']['supplier_retur_id'];
					if($supplierReplace['SupplierReplace']['supplier_replace_status_id']==status_supplier_replace_approved_id){
						$journal = $this->InventoryLedger->insert_to_journal_csv($doc, $id, $this->RecordReferer->getIdByModelNameAndRefName('JournalGroup', 'journal_group_supplier_replace_id'));
						$this->generateCsv($journal);
			...
			****************************/
			...
			if( !empty( $returDetails ) ) {
				...
				if($retur['Retur']['retur_status_id']==status_retur_approved_by_branch_head_id){
					$journal = $this->InventoryLedger->insert_to_journal_csv($doc, $id, $this->RecordReferer->getIdByModelNameAndRefName('JournalGroup', 'journal_group_retur_id'));
					$this->generateCsv($journal);
				}else{
					$this->redirect(array('action' => 'process_inv'));
				}
	...
	function movement_report($asset_category=null) {
		$rows = array();
		$layout=$this->data['InventoryLedger']['layout'];
		if (!empty($this->data['InventoryLedger']['item_id'])) {
	...
	function report_in_recap(){
		...
		//$cc 			= $this->InventoryLedger->query('select a.id,a.name,b.t24_dao from cost_centers a left join cost_center_to_daos b on b.cost_center_id = a.id where a.remarks = "Y" order by a.name');
		//foreach($cc as $data){
		//	if($data[0]['t24_dao']){
