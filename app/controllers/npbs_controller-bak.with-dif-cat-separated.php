...
class NpbsController extends AppController {
      ...
      function index($npb_status_id=null, $is_done=null, $no=null, $request_type_id=null, $department_id=null, $business_type_id=null, $cost_center_id=null /* , $request_type_id=null */) {
            ...
            $this->Session->write('Npb.is_approval2', $this->Session->read('Security.permissions') == 104 ? true : false);
            $this->Session->write('Npb.is_approval2', $this->Session->read('Security.permissions') == 105 ? true : false);
            ...
            if ($group_id == admin_group_id)//admin
                  ...
                  $conditions = array(
					  ...
					  'OR' => array(	  
                           array('npb_status_id' => '2'),
                           array('npb_status_id' => '3'),
                           array('npb_status_id' => '121'),
					)
				);
				  
			else if ($group_id == '104')//approval 2
                  $conditions = array(
                      'AND' => array(
					  'OR' => array(	  
                           array('npb_status_id' => '2'),
                           array('npb_status_id' => '121'),
                           array('npb_status_id' => '123'))
					)
                  );
				  
			else if ($group_id == '105')//approval 3
                  $conditions = array(
                      'AND' => array(
					  'OR' => array(	  
                           array('npb_status_id' => '121'),
                           array('npb_status_id' => '122'),
                           array('npb_status_id' => '123'),
                           array('npb_status_id' => '124'))
					)
                  );
				...
				if(isset($this->data['Npb']['npb_id'])){	
					...
					foreach ($npb_id_updates as $npb_id_update){
						if($npb_id_update > 0){
							...
							$this->data['Npb']['npb_status_id'] = 50;
            ...
            $npbStatuses = $this->Npb->NpbStatus->find('list', array('conditions' =>array('id !=' => status_npb_archive_id))); //archive);
      ...
      function view($id = null,$no_procces=null, $approved_history=null) {
            ...
            $this->Session->write('Npb.is_approval2_head', $group_id == 104 ? true : false);
            $this->Session->write('Npb.is_approval3_head', $group_id == 105 ? true : false);
 ...
 */            //set up session npb.business_type_id
            ...
            if ($group_id == normal_user_group_id) {
				...
				//if ($request_type_id != request_type_stock_id)
					...
					if ($npb_status_id == status_npb_branch_approved_id && $request_type_id != request_type_fa_it_id) {
                        ...
                        $this->Session->write('Npb.can_cancel', true);
						if($request_type_id == request_type_stock_id || $request_type_id == request_type_service_id){
                        $this->Session->write('Npb.can_set_process_type', false);
						}else{
                        $this->Session->write('Npb.can_set_process_type', true);
						}
				...
				if ($request_type_id == request_type_stock_id)
                  ...
                  $this->Session->write('Npb.can_edit', $npb['Npb']['created_by'] == $username
                          && $npb_status_id == status_npb_draft_id
                          && ($request_type_id == request_type_stock_id || $request_type_id == 5)
                  );
            ...
            elseif ($group_id == stock_supervisor_group_id) {
                  if (($request_type_id == request_type_stock_id || $request_type_id == 5) && $npb_status_id == status_npb_sent_to_supervisor_id) {
                        /// NPB Reject status_npb_sent_to_branch_head_id login branch_head_group_id
                        $this->Session->write('Npb.can_reject', true);
      ...
      function approvalDepartmentHead($id = null) {
		...
		function sendEmail($emailType = null, $emailParam = null){
			...
			$content .= '<br>';
			if($emailType == 1){
				$content .= 'has been SUBMITTED by '.$emailParam['requester'].'<br>';
			}else{
			...
			if($emailType == 1){
				if($id_group == 6){
					$userList = $this->Npb->query('select email from users where group_id = 2 and department_id = "'.$res['Npb']['department_id'].'"');
				}else if($id_group == 2){
					$userList = $this->Npb->query('select email from users where group_id = 104 and department_id = "'.$res['Npb']['department_id'].'"');
				}else if($id_group == 104){
					$userList = $this->Npb->query('select email from users where group_id = 105 and department_id = "'.$res['Npb']['department_id'].'"');
				}				
			}else{
			...
			foreach($userList as $user){
				...
				$this->Email->smtpOptions = array(
						'port'=>'',
						'timeout'=>'30',
						'host'=>'smtprelay.ap.rabonet.com',
						'username'=>'',
						'password'=>'');
				$this->Email->to		= $user[0]['email'];
				$this->Email->subject	= 'MR '.$emailParam['no'].' status';
				$this->Email->from		= 'famsys@rabobank.com';
	  ...
	  function can_approve($level,$id) {
		...
		if($level == 3){
			...
			if ($count >= 1) {
				...
				if($reqTypeId == 5){
					...
					$arrayLock 		= array(390,391,392,393,394,395,396,397,398,399,400,401,425,7048,7049,7050,7051,7052,7053);
					$cats = array_values(array_unique($cats));
					$newId1 = $id + 1;
					$newId2 = $id + 2;
					$newId3 = $id + 3;
		...
		$npbs = $this->Npb->read(null, $id);
		foreach($npbs as $npb){
			if ($level == '2' || $level == '121' || $level == '122' || $level == '100'){
				$this->data['Npb']['approved_date'] = date('Y-m-d H:i:s');
				$this->data['Npb']['approved_by']   = $this->Session->read('Userinfo.username');
			}
			if ($id_group == '2'){
	...
	function approve_npb($level,$id) {
		...
		if($reqTypeId == 5){
			$level = 125; //if point reward, send to rac
		}	
		$this->data['Npb']['npb_status_id']=$level;
		...
		if ($id_group == 2){
			...
			$this->data['Npb']['approved_by']   = $this->Session->read('Userinfo.username');
			$this->data['Npb']['approver_one']   = $this->Session->read('Userinfo.username');
			$this->data['Npb']['approver_one_date'] = date('Y-m-d');
		}else if($id_group == 104){
			$this->data['Npb']['approver_two']   = $this->Session->read('Userinfo.username');
	...
	function get_approve_link($id, $npb, $id_group)
	{
		...
		if($auth_amount > $total*$rp_rate && $total*$rp_rate >= 0 
			&& $npb['Npb']['npb_status_id']=='3' && $id_group  == '2')
		{
			$approveLink = array('label'=>__('Finish MR',true),
			'options'=>array('action' => 'approve_npb/50' , $id)); 		
		}
		else if($auth_amount > $total*$rp_rate && $total*$rp_rate > 0 
			&& $npb['Npb']['npb_status_id']=='2' && $id_group  == '104')
		{
			$approveLink = array('label'=>__('Finish MR',true),
			'options'=>array('action' => 'approve_npb/50' , $id)); 		
		}
		else if($auth_amount > $total*$rp_rate && $total*$rp_rate > 0 
			&& $npb['Npb']['npb_status_id']=='121' && $id_group  == '105')
		{
			$approveLink = array('label'=>__('Finish MR',true),
			'options'=>array('action' => 'approve_npb/50' , $id)); 		
		}
		...
		{
			if($id_group  == '6' && $npb['Npb']['npb_status_id']=='1')
			{
				$approveLink = array('label'=>__('Request Approval',true), 
				
				'options'=>array('action' => 'can_approve/3' , $id));
			}
			else if($id_group  == '2' && $npb['Npb']['npb_status_id']=='3')
			{
				$approveLink = array('label'=>__('Approve MR Level 1',true),
				'options'=>array('action' => 'approve_npb/2' , $id)); 
			}
			else if($id_group  == '104' && $npb['Npb']['npb_status_id']=='2')
			{
				$approveLink = array('label'=>__('Approve MR Level 2',true),
				'options'=>array('action' => 'approve_npb/121' , $id)); 
			}
			else if($id_group  == '105' && $npb['Npb']['npb_status_id']=='121')
			{
