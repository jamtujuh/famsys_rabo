				...
				if (isset($this->data['Outlog']['npb_id']) && $detailCount >= 0) {
					...
					foreach ($npb['NpbDetail'] as $npbDetail) {
						...
						foreach($NpbDetailReference as $reference){
							...
							if ($npbDetail['id'] == $reference[0]['NpbDetailId'] && 
								$reference[0]['QtyValue'] > 0 && 
								isset($itemBalances[$npbDetail['item_id']]) &&
								$itemBalances[$npbDetail['item_id']] >= $reference[0]['QtyValue']) {
								...
								$data['OutlogDetail']['qty'] 		= $reference[0]['QtyValue'];									
					...
					if (isset($this->data['Outlog']['npb_id']) && $detailCount < 1) {  
						...
						foreach ($npb['NpbDetail'] as $npbDetail) {
							...
							if ($npbDetail['qty_unfilled'] > 0 && isset($itemBalances[$npbDetail['item_id']])
								...
								$journal = $inLed->insert_to_journal_csv('outlog', $outlog_id, journal_group_distribute_hq_branch_id);
						...
						$journal = $InventoryLedger->insert_to_journal('outlog', $outlog_id, journal_group_distribute_hq_branch_id);
						$this->Outlog->Npb->checkDone($npb_id);
						//echo '6';die();
					}
					return true;
