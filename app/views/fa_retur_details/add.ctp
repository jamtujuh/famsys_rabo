...
<?php
	echo $this->Form->create('FaReturDetail', array('action'=>'add'));
...
<?php echo $this->Form->create('FaReturDetail');?>
	<fieldset>
 		<legend><?php __('Add Fa Retur Detail'); ?></legend>
	<?php
		echo $this->Form->input('fa_retur_id', array('type'=>'hidden','value'=>$this->Session->read('FaRetur.id')));
		echo $this->Form->input('FaRetur.no', array('value'=>$fa_retur_no, 'readonly'=>true));
			...
			<tr<?php echo $class;?>>
				...
				<?php echo $this->Form->checkbox('asset_detail_id', 
					array(
						...
						'name'=>'data[FaReturDetail][asset_detail_id]['.$i.']'
						) )
				?>
				</td>
