...
<div class="houseKeeping view">
	...
	<tr>
		...
		<th><?php echo __('DO Detail')	;?></th>
	</tr>
	
	<?php
		echo $this->Form->create('HouseKeeping', array('action'=>'process_delivery_orders'));
		$i = 0;
		$n = 0;
	...
	?>
		<tr<?php echo $class;?>>
			...
			<td class="left">
				<?php if($d['id'] != null){ ?>
					<?php echo $this->Html->link($d['no'], array('controller' => 'delivery_orders', 'action' => 'view', $d['id']));?>
				<?php }; ?>
			</td>
			<td class="left">
				<?php if($d['id'] != null){?>
					Status: <b><?php echo $d['do_status_name'];?></b><br>
					Created: <?php echo $this->Time->format(DATE_FORMAT, $d['do_date']); ?><br>
