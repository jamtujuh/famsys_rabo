...
<div class="houseKeeping view">
	...
	<tr>
		...
		<th><?php echo __('MR Status')	;?></th>
		<th><?php echo __('Outstanding')	;?></th>		
		<th><?php echo __('PO No')	;?></th>
		<th><?php echo __('PO Detail')	;?></th>
	</tr>
	
	<?php
		echo $this->Form->create('HouseKeeping', array('action'=>'process_pos'));
		$i = 0;
		$n = 0;
		...
		<tr<?php echo $class;?>>
			...
			<td class="left">
				<?php if($d['npb_id'] != null){ ?>
					<?php echo $this->Html->link($d['npb_no'], array('controller' => 'npbs', 'action' => 'view', $d['npb_id']));?>
				<?php }; ?>
			</td>
			<td class="left">
				<?php if($d['npb_id'] != null){?>
					<b><?php echo $d['npb_status_name'];?></b>
				<?php }; ?>
			</td>
			<td class="center">
				<?php if($d['npb_id'] != null){?>
			...
			<td class="left">
				<?php if($d['id'] != null){ ?>
					<?php echo $this->Html->link($d['no'], array('controller' => 'pos', 'action' => 'view', $d['id']));?>
				<?php }; ?>
			</td>
			<td class="left">
				<?php if($d['id'] != null){?>
					Status: <b><?php echo $d['po_status_name'];?></b><br>
					Created: <?php echo $this->Time->format(DATE_FORMAT, $d['po_date']); ?><br>
