...
$request_type_id = 3;
?>
<div id="moduleName"><?php echo 'REPORTS > Stock > Stock Outlog Reports'?></div>
<div id="close_filter_button"><?php echo $this->Html->link(__('Show/Hide Filters',true), '#', array('onclick'=>'$("filter").toggle()')) ?></div>
<div id="filter">
	<div class="fieldfilter">
	<?php echo $this->Form->create('InventoryLedger', array('action'=>'report_out_recap')) ?>
<fieldset>
	...
	<?php echo $this->Form->input('doc_no', array( 'empty'=>'All', 'options'=>array(1=>'Memo Request', 2=>'Retur'))) ?> 
	...
	<?php if ($this->Session->read('Security.permissions') == normal_user_group_id || $this->Session->read('Security.permissions') == branch_head_group_id) : ?>
		  <?php echo $this->Form->input('department_id', array('options' => $departments, 'type' => 'hidden', 'value' => $this->Session->read('Userinfo.department_id'))); ?>
		  <?php echo $this->Form->input('department_name', array('style' => 'width:100%', 'type' => 'text', 'readonly' => true, 'value' => $this->Session->read('Userinfo.department_name'))); ?>
	<?php else : ?>
...
<div class="InventoryLedger related">
	...
	<tr<?php echo $class;?>>
		<td><?php echo $i; ?>&nbsp;</td>
		<td class="left"><?php echo $inventoryLedger['Npb']['no']; ?>
