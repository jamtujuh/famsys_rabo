...
<div class="actions">
	...
	<ul>
		<li> 
			<!--?php echo $this->Html->link(__('Generate Monthly Journal CSV', true), array('controller' => 'journal_transactions', 'action' => 'montly_journal'), array('target'=>'_blank')); ?--> 
			<br/>
			<?php echo $this->Html->link(__('Generate Journal Entry CSV [FT]', true), array('controller' => 'journal_transactions', 'action' => 'journal_entry/ft'), array('target'=>'_blank')); ?> 
			<br/>
			<em>Info: <br/>Please turn off POPUP BLOCKER before Click "Generate"</em>
		</li>
	</ul>
</div>
...
<div class="related">
	...
	foreach ($journalTransactions as $journalTransaction):
		...
		$b = '';
		$c = '';
		if(strstr($journalTransaction['JournalTransaction']['account_code'], '.')){
