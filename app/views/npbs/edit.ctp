...
<?php echo $this->Form->create('Npb');?>
	<fieldset>
 		<legend><?php __('Edit Npb'); ?></legend>
	<?php
		echo $this->Form->input('id', array('type'=>'hidden', 'readonly'=>true) );
		echo $this->Form->input('no', array('type'=>'text', 'style'=>'width:165px', 'readonly'=>true));
		echo $this->Form->input('npb_date', array('value'=>date("Y-m-d"), 'type'=>'text', 'readonly'=>true));
		echo $this->Form->input('request_type_id', array('value'=>$this->data['Npb']['request_type_id'], 'readonly'=>true, 'type'=>'hidden'));
		echo $this->Form->input('request_type', array('value'=>$requestTypes[$this->data['Npb']['request_type_id']], 'readonly'=>true, 'type'=>'text'));
		...
		echo $this->Form->input('cost_center_id', array('empty'=>'select cost center','options'=>$costCenters, 'value'=>$this->data['Npb']['cost_center_id'],));
		echo $this->Form->input('business_type_id', array('type'=>'hidden', 'value'=>$Userinfo['business_type_id']));
		/* echo $this->Form->input('branch', array('style'=>'width:98%','type'=>'text','readonly'=>true, 'value'=>$departments[$Userinfo['department_id']]));
		if(!empty($Userinfo['department_sub_id'])) {
		echo $this->Form->input('department_sub_id', array('type'=>'hidden'));
		...
		echo $this->Form->input('department_unit_id', array('type'=>'hidden'));
...
<div class="actions">
	...
	<ul>
		...
		<li><?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $this->Form->value('Npb.id')), null, sprintf(__('Are you sure you want to delete # %s?', true), $this->Form->value('Npb.id'))); ?></li>
		<li><?php echo $this->Html->link(__('List Npbs', true), array('action' => 'index'));?></li>
		<li><?php echo $this->Html->link(__('New Npb', true), array('action' => 'add')); ?></li>
	</ul>
</div>