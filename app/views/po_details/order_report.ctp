...
<div id="filter">
	...
	<fieldset>
			...
			<legend><?php __('Item Order Filters') ?></legend>
			<fieldset class='subfilter'>
			<legend><?php __('PO Info')?></legend>
			<?php echo $form->input('asset_category_type_id', array('options' => $assetCategoryTypes, 'empty' => '', 'value' => $this->Session->read('PoDetail.asset_category_type_id'))); ?>
...
<?php if (!empty($poDetails)): ?>
      <div class="related">
            <h2><?php __('Item Ordered List'); ?></h2>
            <table cellpadding="0" cellspacing="0">
                  ...
                  foreach ($poDetails as $poDetail):
                        ...
                        $total+=$poDetail['PoDetail']['amount_cur'];
                        $places = $myApp->getPlaces($currency[$poDetail['Po']['currency_id']]);

                        ?>
                        <tr<?php echo $class; ?>>
                              <td><?php echo $i; ?>&nbsp;</td>
                              <td  class="left">
                                    <?php echo $this->Html->link($poDetail['Po']['no'], array('controller' => 'pos', 'action' => 'view', $poDetail['Po']['id'])); ?>
                              </td>
                              <td class="left"><?php echo $this->Time->format(DATE_FORMAT, $poDetail['Po']['po_date']); ?>&nbsp;</td>
                              <td class="left"><?php echo $poDetail['Department']['name']; ?></td>
                              ...
                              <td class="center"><?php echo $currencies[$poDetail['Po']['currency_id']]; ?></td>
                  ...
                  <?php endforeach; ?>
                  <!--tr>
                        <td colspan="12"  class="number"><?php __('Total') ?></td>
